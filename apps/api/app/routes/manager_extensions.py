...
def list_sessions(
    ...
    rows = db.execute(
        text(
            ...
            "'id', l.id, 'code', l.lecturer_code, 'name', a.display_name, "
            "'role', COALESCE(msr.role_key, cm.assignment::text), "
            "'roleLabel', CASE "
            "  WHEN msr.role_key = 'CHAIR' THEN 'Chủ tịch' "
            "  WHEN msr.role_key = 'SECRETARY' THEN 'Thư kí' "
            "  WHEN msr.role_key LIKE 'MEMBER_%' THEN 'Thành viên ' || split_part(msr.role_key, '_', 2) "
            "  WHEN msr.role_key LIKE 'REVIEWER_%' THEN 'Review ' || split_part(msr.role_key, '_', 2) "
            "  ELSE COALESCE(msr.role_key, cm.assignment::text) "
            "END, "
            "'order', msr.role_order"
            ")) FILTER (WHERE l.id IS NOT NULL) AS reviewers "
            ...
            "LEFT JOIN manual_schedule_sessions mss ON mss.published_session_id = s.id "
            "LEFT JOIN manual_schedule_session_"
            "reviewers msr ON msr.session_id = mss.id AND msr.lecturer_id = l.id "
