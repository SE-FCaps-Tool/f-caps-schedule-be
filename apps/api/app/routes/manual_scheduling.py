...
def _load_manual_sessions(db: Session, round_id: int) -> list[dict[str, Any]]:
    ...
    group_rows = db.execute(
        text(
            ...
            "LEFT JOIN LATERAL ( "
            "  SELECT COALESCE(a.display_name, st.full_name) AS display_name FROM group_memberships gm "
            "  JOIN students st ON st.id = gm.student_id LEFT JOIN accounts a ON a.id = st.account_id "
