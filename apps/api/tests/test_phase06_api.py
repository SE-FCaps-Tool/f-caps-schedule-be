...
def test_manager_can_record_defense_result_and_advance_group(client):
    ...
    round_response = client.post(
        "/api/v1/rounds",
        json={"semester_id": semester_id, "type": "REVIEW_3", "reviewer_count": 3, "room_types": ["NORMAL"], "session_duration_minutes": 30, "startDate": "2052-04-01", "endDate": "2052-04-01"},
        headers=headers,
    )
    ...
    day = client.post(
        ...
        json={"day_date": "2052-04-01", "slots": [{"start_at": "2052-04-01T09:00:00+07:00", "end_at": "2052-04-01T09:30:00+07:00"}]},
        headers=headers,
    )
    ...
    assert client.post(f"/api/v1/rounds/{round_id}/resources", json={"groupIds": [group["id"]], "timeslotIds": [slot_id], "room_ids": [room["id"]]}, headers=headers).status_code == 200
    for lecturer in client.get("/api/v1/lecturers", headers=headers).json()["data"][:6]:
        assert client.post(f"/api/v1/rounds/{round_id}/lecturers/{lecturer['id']}/availability", json={"selectedTimeslotIds": [slot_id]}, headers=headers).status_code == 200
    ...
    run = client.post(f"/api/v1/rounds/{round_id}/schedule/run", json={"timeLimitSeconds": 2}, headers=headers)
    ...
    result = client.post(f"/api/v1/sessions/{session_id}/result", json={"result": "LEVEL_1", "note": "Pass to Defense 1"}, headers=headers)
    assert result.status_code == 201, result.text
    assert result.json()["data"]["groupStatus"] == "ELIGIBLE_D12"
