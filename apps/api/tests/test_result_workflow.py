...
def test_review_3_accepts_four_levels(outcome):
    assert validate_result_outcome("REVIEW_3", outcome) is True


def test_defense_1_is_completion_only_and_defense_2_is_binary():
    assert validate_result_outcome("DEFENSE_1", "COMPLETED") is True
    assert validate_result_outcome("DEFENSE_2", "PASS") is True
    with pytest.raises(DomainError, match="OUTCOME_NOT_ALLOWED"):
        validate_result_outcome("DEFENSE_1", "LEVEL_1")
    with pytest.raises(DomainError, match="OUTCOME_NOT_ALLOWED"):
