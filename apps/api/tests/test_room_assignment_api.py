...
def test_assign_suggest_apply_are_planned_only_atomic_and_idempotent(client):
    ...
    pairs = suggestion.json().get("suggestions", suggestion.json())
    assert isinstance(pairs, list)
    if not pairs:
        pytest.skip("seed fixture has no active PLANNED sessions")

    pair = {"session_id": pairs[0]["session_id"], "room_id": pairs[0]["room_id"]}
    applied = client.post(
        "/api/v1/rounds/1/rooms/apply-suggestions",
    ...
    assert applied.json().get("changed_count", 0) in {0, 1}
