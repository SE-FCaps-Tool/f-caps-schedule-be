...
def test_semester_list_detail_and_filters_expose_complete_contract(client):
    ...
    filtered = client.get(
        "/api/v1/semesters",
        params={"status": item["status"], "academic_year": item["academic_year"]},
        headers=MANAGER_HEADERS,
    )
    assert filtered.status_code == 200
    assert all(row["status"] == item["status"] for row in filtered.json())
    assert all(row["academic_year"] == item["academic_year"] for row in filtered.json())


@pytest.mark.integration
def test_semester_create_patch_and_set_current_are_atomic(client):
    before = client.get("/api/v1/semesters", headers=MANAGER_HEADERS).json()
